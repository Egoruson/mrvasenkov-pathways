...
const Gradient: React.FC<{ gradients: GradientType[]; animationDuration: number }> = React.memo(({ gradients, animationDuration }) => {
...
const DotCanvas: React.FC<{
  dotSize: number; dotSpacing: number; dotOpacity: number; waveIntensity: number; waveRadius: number
  dotColor: string; glowColor: string; performance: "low" | "medium" | "high"; mousePos: { x: number; y: number }
  ...
  const drawDots = useCallback((ctx: CanvasRenderingContext2D, time: number) => {
    ...
    const skip = performance === "low" ? 3 : performance === "medium" ? 2 : 1
  ...
  return (
    <canvas
      ...
      style={{ mixBlendMode: "multiply", backgroundColor: "hsl(0 0% 96%)" }}
    />
  )
})
DotCanvas.displayName = "DotCanvas"

const MouseGlow: React.FC<{ glowColor: string; mousePos: { x: number; y: number } }> = React.memo(({ glowColor, mousePos }) => (
  <>
    <div
...
export function CanvasFractalGrid({
  dotSize = 4, dotSpacing = 20, dotOpacity = 0.3, gradientAnimationDuration = 20,
  waveIntensity = 30, waveRadius = 200, gradients = defaultGradients,
  dotColor = "rgba(100, 100, 255, 1)", glowColor = "rgba(100, 100, 255, 1)",
  enableNoise = true, noiseOpacity = 0.03, enableMouseGlow = true,
  initialPerformance = "medium", enableGradient = false,
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    if (!rect) return
    setMousePos({ x: (event.clientX - rect.left) / rect.width, y: (event.clientY - rect.top) / rect.height })
  }, [])
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <DotCanvas
          dotSize={responsiveDotSize} dotSpacing={responsiveDotSpacing} dotOpacity={dotOpacity}
          waveIntensity={waveIntensity} waveRadius={waveRadius} dotColor={dotColor}
          glowColor={glowColor} performance={performance} mousePos={mousePos}
        />
